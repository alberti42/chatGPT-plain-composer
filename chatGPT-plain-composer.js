...
// @version      0.5
// @description  Replace ChatGPT composer with a plain textarea for smoother typing. Adds autogrow + drafts + cleanup + correct multiline sending + throttled MutationObserver + non-overlapping toggle UX.
...
(function () {
  ...
  function ensureReturnButton() {
    ...
    // Styling: small floating button bottom-right
    ...
    btn.addEventListener("click", () => {
      // User wants plain composer back
  ...
  async function sendPlainMessage() {
    ...
    // If user is in "original visible mode", don't force-hide it after sending.
    // Otherwise restore our typical hidden state.
  ...
  function installIfNeeded() {
    ...
    // Only auto-hide original if user is not intentionally viewing it
  ...
  function run() {
    ...
    log("Initialized v0.5 (toggle UX + throttling).", {
